...
RESULTS_DIR = os.path.join(REPO_ROOT, "results", "sequence")
MODELS_DIR = os.path.join(REPO_ROOT, "models", "sequence")
...
def make_loader(dataset, labels, starts, batch_size, balanced):
    if not balanced:
        return DataLoader(dataset, batch_size=batch_size, shuffle=True, pin_memory=True)

    seq_labels = labels[starts]
    counts = np.bincount(seq_labels, minlength=N_CLASSES).astype(np.float32)
    weights = 1.0 / np.maximum(counts, 1.0)
    sample_weights = torch.tensor(weights[seq_labels], dtype=torch.double)
...
def train(args):
    ...
    train_loader = make_loader(train_ds, y_train_ds, train_starts, args.batch_size, args.balanced_sampler)
...
def parse_args():
    ...
    parser.add_argument("--stride", type=int, default=16, help="Training sequence stride in frames.")
    ...
    parser.add_argument("--rms-threshold", type=float, default=100.0)
    ...
    parser.add_argument(
        ...
        default="audio1_val_audio2_test",
