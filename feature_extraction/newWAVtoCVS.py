...
RMS_THRESHOLD = 50.0  # int16 units; chunks below this are treated as silent
...
def extract_chunks(base, angle, duration):
    ...
    # find onset: first chunk where avg RMS across all 4 mics exceeds threshold
    onset = 0
    for ci in range(n_chunks):
        s = ci * chunk_samples
        e = s + chunk_samples
        avg_rms = float(np.mean([np.sqrt(np.mean(signals[m][s:e] ** 2)) for m in MICS]))
    ...
    for ci in range(onset, n_chunks):
        s = ci * chunk_samples
        e = s + chunk_samples
    ...
    for ds_name, base in DATASETS.items():
        ...
        for angle in ANGLES:
            ...
            if chunks is None:
                ...
                kept, total, onset = chunks
                samples.extend(kept)
                print(f'  {angle:>3}°  ->  {len(kept)} chunks  (onset @ chunk {onset}/{total})')
