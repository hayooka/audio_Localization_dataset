...
FEATURE_COLS = [
    ...
    *[f'logmel_{mic}_b{b}' for mic in _MICS for b in range(_N_MELS)],
    #*[f'mfcc_{mic}_c{c}' for mic in _MICS for c in range(_N_MFCC)],
]
...
# ── Plot — S2 only (1x2: accuracy + loss) ────────────────────────────────────
r = results['S2: 80% train -> test']
epochs_x = list(range(1, len(r['tr_hist']) + 1))
best_ep  = r['te_hist'].index(max(r['te_hist'])) + 1

fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 5))
fig.suptitle(f'Localization CNN — {CHUNK_TAG}ms  |  Acc={r["acc"]:.1f}%  MAE={r["mae"]:.1f}deg', fontsize=12)

ax1.plot(epochs_x, [v * 100 for v in r['tr_hist']], label='Train')
ax1.plot(epochs_x, [v * 100 for v in r['te_hist']], label='Test')
...
ax2.plot(epochs_x, r['loss_hist'], color='coral')
ax2.axvline(best_ep, color='gray', linestyle='--', alpha=0.6, label=f'Best epoch ({best_ep})')
